...
const API_KEY = "live_OIUon2jzstQQlCFHyOosCnnIUw9PrSPHWZGuIYYCWUWatYLSMn7Imvt9WpQueY7D";
...
async function initialLoad(){
    const res= await fetch('https://api.thecatapi.com/v1/breeds',
        {headers:{'x-api-key': API_KEY}} //headers are to add extra information needed 
        //Authenticating using the request header To send an authenticated request, you need to send the header parameter x-api-key in the request's header.        headers:{ 'x-api-key' : 'YOUR_API_KEY' }
    )
...
async function handleBreedSelect() {
   ...
   //Todo more create a new element for the info
...
async function axiosHandleBreedSelect() {
    ...
    //fetching data by breed id using axios not fetching
    console.log(breedSelect.value);
...
export async function favourite(imgId) {
        console.log(imgId);}
//delete everything if you already have favourites
// axios
...
//     );
   ...
   *    If that isn't in its own function, maybe it should be so you don't have to repeat yourself in this section.
   */
